...
class linked_list:
    ...
    def insert_at(self, data, index):
        ...
        left_node = self.root_node if index == 0 else self.get_node_at(index - 1)
        if left_node is None:
            raise IndexError("IndexError")
        
        if index == 0:
            new_node.next_node = self.root_node
            self.root_node = new_node
        else:
            next_next_node = left_node.next_node
            left_node.next_node = new_node
            new_node.next_node = next_next_node
